...
def weight_init(shape):
    ...
    # 5,5 表示卷积核的大小, 1代表通道channel（对彩色图片做卷积是3,这里单色灰度为1)
...
# 池化
    # 卷积核在提取特征时的动作成为padding，它有两种方式：SAME和VALID
...
b1 = bias_init([16])
conv_1 = conv2d(x_input,w1)+b1 # 结果 NHWC  N H W C
...
f_w3 = fch_init(128,2)
f_b3 = bias_init([2])
f_r3 = tf.matmul(f_dropout_r2,f_w3) + f_b3  #最后输出结果，可能是这样的[[0.0001,0.99999] ，那个位置的结果大就属于哪个分类
# 分类器是softMax分类器
...
arg2 = tf.argmax(f_softmax,1)   #每个样本的预测结果是一个(1,2)的vector
cos = tf.equal(arg1,arg2)
acc = tf.reduce_mean(tf.cast(cos,dtype=tf.float32))     #tf.cast把bool值转换为浮点数
